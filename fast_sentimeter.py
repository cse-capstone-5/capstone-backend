...
def get_linechart():
    ...
    for row in sent_data.itertuples():
        tmp = {"name":str(row[0]), "sent_val":row[1], "pos_count":row[2], 'neg_count':row[3]}
        lineChart.append(tmp)
    return lineChart

# %%
async def crawl_sent_analysis(keyword,ds,de,limit):
    global news_data, limit_per_day
    limit = int(limit)
    if (limit>=10 and limit<=100):
        limit_per_day = limit
    ...
    result = requests.post('http://175.210.144.82:8049/sentimeter',data=news_data.to_json(),headers=headers)
    news_data = pd.read_json(json.loads(result.text))
    # print(news_data)
    news_data['date'] = pd.to_datetime(news_data['date'])
    # print("done")
...
# %%
#await crawl_sent_analysis('전세','20230201','20230202','1')

# %%
if __name__ == '__main__':
    #await crawl_sent_analysis(crawl_sent_analysis(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4]))
    asyncio.run(crawl_sent_analysis(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4]))
